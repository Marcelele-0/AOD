...
\begin{itemize}
    ...
    \item $k$ - zasięg widzenia kamery (liczba kratek w cztery strony świata)
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Pokrycie monitoringu:} Każdy kontener musi być obserwowany przez co najmniej jedną kamerę. Niech $V_{(r,c)}$ oznacza zbiór pól, z których kamera o zasięgu $k$ widzi pole $(r,c)$.
    $$ \sum_{(i, j) \in V_{(r,c)}} x_{ij} \ge 1 \quad \forall (r, c) \in Q $$
    Zbiór $V_{(r,c)}$ definiujemy jako pola $(i,j)$ spełniające warunek:
    $$ (i=r \land |j-c| \le k) \lor (j=c \land |i-r| \le k) $$
\end{enumerate}

\subsection{Wyniki eksperymentu}
Wygenerowano instancję problemu o wymiarach $6 \times 6$ ($m=6, n=6$) z 6 losowo rozmieszczonymi kontenerami. Przetestowano dwa warianty parametru $k$. Poniżej przedstawiono bezpośredni wydruk z programu (mapy rozmieszczenia):

\begin{verbatim}
DANE: Teren 6x6, Liczba kontenerów: 6

========================================
   ROZWIĄZYWANIE DLA ZASIĘGU k = 1
========================================
...
\bigskip
\noindent \textbf{Wariant 1: Zasięg $k=1$}
\begin{itemize}
    \item \textbf{Minimalna liczba kamer:} 6
    \item \textbf{Interpretacja:} Przy małym zasięgu kamery muszą być ustawione bardzo blisko kontenerów (często w relacji 1:1), co wymusza użycie dużej liczby urządzeń.
\end{itemize}

\noindent \textbf{Wariant 2: Zasięg $k=2$}
\begin{itemize}
    \item \textbf{Minimalna liczba kamer:} 3
    \item \textbf{Rozmieszczenie:} (3, 1), (4, 5), (6, 2).
    \item \textbf{Interpretacja:} Zwiększenie zasięgu pozwoliło na drastyczną redukcję liczby kamer (o 50\%). Jedna kamera, dzięki dłuższemu zasięgowi w pionie i poziomie, jest w stanie monitorować wiele rozproszonych kontenerów jednocześnie (np. kamera w (3,1) widzi kontenery w (1,1) i (3,3)).
\end{itemize}
